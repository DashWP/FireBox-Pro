...
class Box
{
		...
		add_action('wp_enqueue_scripts', function() use ($fbox, $css) {
			...
			if ($fbox->params->get('triggermethod') === 'floatingbutton' || $fbox->params->get('floating_button_show_on_close', '0'))
			{
				$floating_button_vars = [
					...
					'fontSize' => (int) $fbox->params->get('floatingbutton_message.fontsize', 16) . 'px'
				];
	...
	public function prepare()
	{
        ...
        $this->box->settings = [
			...
			'scroll_depth_value'   => $scroll_depth == 'percentage' ? (int) $this->box->params->get('triggerpercentage') : (int) $this->box->params->get('scroll_pixel'),
