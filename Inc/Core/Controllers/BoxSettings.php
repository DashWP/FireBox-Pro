...
class BoxSettings extends BaseController
{
	...
	public function processBoxSettings($input)
	{
        ...
        {
            $license_key = '';
            if (isset($input['license_key']) && !empty($input['license_key']) && isset($input['license_update_btn']) && !empty($input['license_update_btn']))
            {
        ...
        else if (isset($input['license_update_btn']) && $input['license_update_btn'] == 'Deactivate License')
        {
    ...
    private function validateAndActivateLicense($license_key)
    {
        ...
        $old_license_key = get_option('firebox_license_key');
    ...
    private function checkAndDeactivateLicense()
    {
        ...
        if (!check_admin_referer('fpf_form_nonce_firebox_settings', 'fpf_form_nonce_firebox_settings'))
        {
            return false; // get out if we didn't click the Deactivate button
        }
